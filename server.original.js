...
const disableCoreDumpsForProcess = () => {
  ...
  // Some shared hosts do not provide `prlimit`; continue without failing startup.
};
...
const startCoreCleanupLoop = () => {
  const intervalMs = Number(process.env.CORE_CLEANUP_INTERVAL_MS || 60000);
  if (!Number.isFinite(intervalMs) || intervalMs <= 0) return;

  const timer = setInterval(cleanupCoreFiles, intervalMs);
